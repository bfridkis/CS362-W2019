 ...
 * Assignment 3
 *
 *                         cardtest_adventurer.c
 *
 *	     Test for "adventurer" card effect - See cardEffects.c line 5
 *
 * Include the following lines in your makefile:
 *
 * cardtest_adventurer: cardtest_adventurer.c _cardtesthelper_adventurer.o dominion.o cardEffects.o rngs.o
 * 		gcc -o cardtest_adventurer cardtest_adventurer.c -g dominion.o cardEffects.o 
 * 		_cardtesthelper_adventurer.o rngs.o $(CFLAGS)
 * ---------------------------------------------------------------------------
 */
...
int main (int argc, char** argv) {
	...
	if(RANDOMIZE){
		...
		printf("\n" "  Executing %d Adventurer %s using deck and hand sizes starting at 6 and incrementing\n"
			   ...
			   "\t" "  hand size < 6, hand size will be set to 6 (see _cardtesthelper_adventurer.c line 90).\n"
			   ...
			   "\t\t" "  in _cardtesthelper_adventurer.h.\n\n", ADVENTURER_CALLS, 
	...
	for(i = 0; i < ADVENTURER_CALLS; i++){
		...
		//(see _cardtesthelper_adventurer for more details)
		_cardtesthelper_adventurer(k, &G, failures, &failCt, 2, 0, i + 1);
	}
	...
	initializeGame(2, k, seed, &G);
	_cardtesthelper_adventurer(k, &G, failures, &failCt, 1, 1, ADVENTURER_CALLS + 1);
	...
	initializeGame(2, k, seed, &G);
	_cardtesthelper_adventurer(k, &G, failures, &failCt, 0, 1, ADVENTURER_CALLS + 2);
	...
	initializeGame(2, k, seed, &G);
	_cardtesthelper_adventurer(k, &G, failures, &failCt, -1, 1, ADVENTURER_CALLS + 3);
	...
	//Print summary of all failed tests (up to MAX_FAILS)
		...
		printf("  (Note: See _cardtesthelper_adventurer.c when referencing line #)\n\n");
		int i;
