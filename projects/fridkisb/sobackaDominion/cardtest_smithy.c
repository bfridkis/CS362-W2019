...
int main (int argc, char** argv) {
	...
	printf("\t\t\t\t\t" "Starting cardtest_smithy - Testing 'smithy' card\n");
	...
	for(i = 0; i < SMITHY_CALLS; i++){
		...
		//(see _cardtesthelper_smithy for more details)
		_cardtesthelper_smithy(k, &G, failures, &failCt, 3, 0, i + 1);
	}
	...
	initializeGame(2, k, seed, &G);
	_cardtesthelper_smithy(k, &G, failures, &failCt, 1, 1, SMITHY_CALLS + 1);
	...
	initializeGame(2, k, seed, &G);
	_cardtesthelper_smithy(k, &G, failures, &failCt, 2, 1, SMITHY_CALLS + 2);
	...
	initializeGame(2, k, seed, &G);
	_cardtesthelper_smithy(k, &G, failures, &failCt, 0, 1, SMITHY_CALLS + 3);
	...
	//Print summary of all failed tests (up to MAX_FAILS)
		...
		printf("  (Note: See _cardtesthelper_smithy.c when referencing line #)\n\n");
		int i;
